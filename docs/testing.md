...
|---|---:|---|
| Web component, API, export, and demo tests | 40 | `npm test` |
| Shared schema and link validation | 10 | `npm test` |
| Solidity analyzer fixtures | 14 | `npm test` |
| Protocol property-engine fixtures | 14 | `npm test` |
...
`npm test` currently covers 115 web, API, package, runner, parser, and eval assertions. Browser E2E checks are listed separately because Playwright runs them against a local production Next server.
